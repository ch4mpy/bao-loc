...
class SolutionsControllerTest {
	...
	void whenGetSolutionsThenFirstPageIsReturned() throws Exception {
		...
		mockMvc.get("/v1/solutions").andExpect(status().isOk()).andExpect(jsonPath("$", hasSize(player.getSolutions().size())));
	}
	...
	void whenGetSolutionByExistingIdThenItIsReturned() throws Exception {
		...
		mockMvc.get("/v1/solutions/1").andExpect(status().isOk()).andExpect(jsonPath("$.x1", is(solution.getX1().intValue())))
	...
	void whenGetSolutionByNonExistingIdThenNotFound() throws Exception {
		...
		mockMvc.get("/v1/solutions/42").andExpect(status().isNotFound());
	}
	...
	void whenPutValidSolutionAtValidIdThenAccepted() throws Exception {
		...
		mockMvc.put(payload, "/v1/solutions/{solutionId}", solution.getId()).andExpect(status().isAccepted());
	}
	...
	void whenPutSolutionWithDuplicatedValuesAtValidIdThenBadRequest() throws Exception {
		...
		mockMvc.put(payload, "/v1/solutions/{solutionId}", solution.getId()).andExpect(status().isBadRequest());
	}
	...
	void whenPutValidSolutionAtInvalidIdThenNotFound() throws Exception {
		...
		mockMvc.put(payload, "/v1/solutions/{solutionId}", 42L).andExpect(status().isNotFound());
	}
	...
	void whenDelteSolutionAtValidIdThenItsActuallyDeleted() throws Exception {
		...
		mockMvc.delete("/v1/solutions/{solutionId}", solution.getId()).andExpect(status().isAccepted());
	...
	void whenDelteSolutionAtInvalidIdThenNotFound() throws Exception {
		...
		mockMvc.delete("/v1/solutions/42").andExpect(status().isNotFound());
	}
}
