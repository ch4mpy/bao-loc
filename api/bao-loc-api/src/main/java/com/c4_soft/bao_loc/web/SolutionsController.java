 ...
 */
@RestController
@RequestMapping(path = "/v1/solutions", produces = { MediaType.APPLICATION_JSON_VALUE, MediaType.APPLICATION_XML_VALUE })
...
public class SolutionsController {
	...
	public ResponseEntity<?> createSolution(@RequestBody @Valid SolutionUpdateRequest dto, OAuthentication<OpenidClaimSet> auth) {
		...
		return ResponseEntity.accepted().header(HttpHeaders.LOCATION, String.format("/solutions/%s", solutionRepo.save(solution).getId())).build();
	}
